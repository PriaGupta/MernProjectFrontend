...
const Card = ({image,name,price,category,loading,id}) => {
  ...
  return (
    <div className='w-full min-w-[180px] max-w-[200px] text-center gap-5 bg-white hover:shadow-2xl drop-shadow-lg py-5 px-4 mb-4 cursor-pointer flex flex-col '>
     
     {
        image ? (<>
        <Link to={`/product/${id}`} onClick={()=>window.scrollTo({top:"0",behavior:"smooth"})} >
         <div className='h-28 flex flex-col justify-center items-center'>
        <img src={image} className='h-full ' alt="cardimg"/> 
      </div>
